document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', (event) => {
      ...
      if (data.nome.length > 255){
        alert('Invalido. Máximo de 255 caracters!');
        return;
      } else if (data.msg.length > 200){
        alert('Invalido. Máximo de 200 caracters!');
        return;
      } else if (data.email.length > 256){
        alert('Invalido. Máximo de 256 caracters!')
        return;
      } else if (!data.celular || data.celular.toString().length > 15 || data.celular.toString().length < 13){
        alert('Invalido. Máximo e Minimo de 15 números!')
        return;
      }
